export const VEHICLE_BRANDS = [
  'Toyota',
  'Honda',
  'Nissan',
  'Ford',
  'Chevrolet',
  'Volkswagen',
  'Hyundai',
  'Kia',
  'Mazda',
  'Suzuki',
  'Mitsubishi',
  'Renault',
  'Peugeot',
  'Citroën',
  'Fiat',
  'Mercedes-Benz',
  'BMW',
  'Audi',
  'Volvo',
  'Jeep',
  'Dodge',
  'Chrysler',
  'Subaru',
  'Isuzu',
  'Great Wall',
  'Chery',
  'Geely',
  'BYD',
  'JAC',
  'MG',
] as const;

export const VEHICLE_MODELS: Record<string, string[]> = {
  Toyota: [
    'Corolla',
    'Camry',
    'RAV4',
    'Hilux',
    'Yaris',
    'Prius',
    'Land Cruiser',
    'Prado',
    'Fortuner',
    'Highlander',
    'Sienna',
    'Tacoma',
    'Tundra',
    '4Runner',
    'Sequoia',
  ],
  Honda: [
    'Civic',
    'Accord',
    'CR-V',
    'Pilot',
    'HR-V',
    'Fit',
    'Odyssey',
    'Passport',
    'Ridgeline',
    'Insight',
    'Ridgeline',
  ],
  Nissan: [
    'Sentra',
    'Altima',
    'Versa',
    'Rogue',
    'Pathfinder',
    'Frontier',
    'Titan',
    'Murano',
    'X-Trail',
    'Kicks',
    'Note',
  ],
  Ford: [
    'F-150',
    'F-250',
    'Explorer',
    'Escape',
    'Edge',
    'Expedition',
    'Mustang',
    'Focus',
    'Fusion',
    'Ranger',
    'Bronco',
  ],
  Chevrolet: [
    'Silverado',
    'Equinox',
    'Tahoe',
    'Suburban',
    'Traverse',
    'Malibu',
    'Cruze',
    'Trailblazer',
    'Blazer',
    'Colorado',
  ],
  Volkswagen: [
    'Jetta',
    'Passat',
    'Tiguan',
    'Atlas',
    'Golf',
    'Polo',
    'Amarok',
    'T-Cross',
    'Nivus',
    'Virtus',
  ],
  Hyundai: [
    'Elantra',
    'Sonata',
    'Tucson',
    'Santa Fe',
    'Kona',
    'Palisade',
    'Accent',
    'Venue',
    'Creta',
    'i20',
  ],
  Kia: [
    'Rio',
    'Forte',
    'Optima',
    'Sorento',
    'Sportage',
    'Telluride',
    'Seltos',
    'Soul',
    'Carnival',
    'Stonic',
  ],
  Mazda: [
    'Mazda3',
    'Mazda6',
    'CX-5',
    'CX-9',
    'CX-30',
    'CX-50',
    'MX-5',
    'BT-50',
  ],
  Suzuki: [
    'Swift',
    'Vitara',
    'S-Cross',
    'Grand Vitara',
    'SX4',
    'Jimny',
    'Ertiga',
    'Celerio',
  ],
  Mitsubishi: [
    'Outlander',
    'Eclipse Cross',
    'Montero',
    'L200',
    'ASX',
    'Mirage',
    'Pajero',
  ],
  Renault: [
    'Duster',
    'Sandero',
    'Logan',
    'Koleos',
    'Captur',
    'Kwid',
    'Oroch',
    'Fluence',
  ],
  Peugeot: ['208', '308', '3008', '5008', 'Partner', 'Expert', '2008'],
  Citroën: ['C3', 'C4', 'C5', 'Berlingo', 'Jumper'],
  Fiat: ['Uno', 'Palio', 'Siena', 'Strada', 'Toro', 'Mobi', 'Argo', 'Cronos'],
  'Mercedes-Benz': [
    'Clase A',
    'Clase C',
    'Clase E',
    'Clase S',
    'GLC',
    'GLE',
    'GLS',
    'Sprinter',
  ],
  BMW: ['Serie 1', 'Serie 3', 'Serie 5', 'Serie 7', 'X1', 'X3', 'X5', 'X7'],
  Audi: ['A3', 'A4', 'A6', 'A8', 'Q3', 'Q5', 'Q7', 'Q8'],
  Volvo: ['XC40', 'XC60', 'XC90', 'S60', 'S90', 'V60', 'V90'],
  Jeep: [
    'Wrangler',
    'Cherokee',
    'Grand Cherokee',
    'Compass',
    'Renegade',
    'Gladiator',
  ],
  Dodge: ['Charger', 'Challenger', 'Durango', 'Ram', 'Journey'],
  Chrysler: ['300', 'Pacifica', 'Voyager'],
  Subaru: ['Forester', 'Outback', 'Crosstrek', 'Ascent', 'Impreza', 'Legacy'],
  Isuzu: ['D-Max', 'MU-X'],
  'Great Wall': ['Haval H6', 'Wingle', 'Cannon'],
  Chery: ['Tiggo', 'QQ', 'Arrizo'],
  Geely: ['Coolray', 'Tugella', 'Atlas'],
  BYD: ['Song', 'Tang', 'Yuan'],
  JAC: ['T40', 'T50', 'T60'],
  MG: ['ZS', 'HS', 'RX5'],
};

export const VEHICLE_COLORS = [
  'Blanco',
  'Negro',
  'Gris',
  'Plata',
  'Rojo',
  'Azul',
  'Verde',
  'Amarillo',
  'Naranja',
  'Marrón',
  'Beige',
  'Dorado',
  'Bordó',
  'Violeta',
  'Rosa',
] as const;
